...
class Passport:
    required_fields = {
        ...
        "hgt": (lambda x: int(x[:-2]) in range(150, 193 + 1) if x[-2:] == "cm" else (int(x[:-2]) in range(59, 76 + 1) if x[-2:] == "in" else False)),
    ...
    def has_required_fields(self):
        match_count = len([field for field in self.required_fields if field in self.fields]) 
        req_count = len(self.required_fields)
        return match_count == req_count
    
    def is_valid(self):
        for key, value in self.required_fields.items():
            if not value(self.fields[key]):
                return False
...
def get_passports(filename):
    ...
    with open(filename, "r") as file:
        ...
        for line in file:
            ...
            for item in items:
                match = re.match("(.+):(.+)", item)
                passport_fields[match.group(1)] = match.group(2)

        # Append final passport.
        passports.append(Passport(passport_fields))
...
def solve_part_two(passports):
    print(len([passport for passport in passports if passport.has_required_fields() and passport.is_valid()]))

passports = get_passports("aoc_2020_04_input")
