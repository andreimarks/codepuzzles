...
def get_final_min(assignment, min, max):
    range = max - min
    half = int(range/2)

    if assignment == "":
        return min
    elif assignment[0] in ["F", "L"]:
        max -= half
    else:
        min += half

    return get_final_min(assignment[1:], min, max)


def get_seat_id(seat):
    codes = [seat[:-3], seat[-3:]]
    assignments = [get_final_min(code, 0, 2 ** len(code)) for code in codes]
